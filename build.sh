...
help() {
    ...
    echo "  -h                  Display help"
    echo "  -C <initial-cache>  CMake initial-cache to use."
    echo "  --no-initial-cache  Do not use a CMake initial-cache."
...
CMAKE_CACHE_OPTION=""
if [ ! -z "${CMAKE_CACHE}" ] && [[ $NO_CMAKE_INITIAL_CACHE -eq 0 ]]; then
    CMAKE_CACHE_OPTION="-C ${CMAKE_CACHE}"
    echo "Using CMake cache: ${CMAKE_CACHE}"
else
    echo "No CMake cache specified."
fi
