 ...
 */
typedef enum { DELAY_HOURS, DELAY_MINS, DELAY_SECS, DELAY_MILLIS } delayUnits;
 ...
 *
 * @return a delayedTask handle (positive number) that can be queried or
 * canceled
 */
uint32_t scheduleDelayTask(uint64_t delayAmount, delayUnits units,
                           taskCallbackFunc func, void *arg);
 ...
 * canceled
 */
uint32_t scheduleTimeOfDayTask(uint8_t hour, uint8_t min, taskCallbackFunc func,
                               void *arg);
