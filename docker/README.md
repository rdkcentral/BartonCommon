# BartonCommonCommon Docker Process

The BartonCommonCommon project uses Docker to create a consistent and reproducible development environment.
...
Additionally, BartonCommon uses Docker Compose to simplify the management of multi-container applications,
...
To build the BartonCommon image, simply run the `docker/build.sh` to create a BartonCommon image.
...
new tag while developing. In BartonCommon, there are several places where this image tag must be known
...
Due to the layout of BartonCommon's docker and devcontainer environment, running the devcontainer commands
